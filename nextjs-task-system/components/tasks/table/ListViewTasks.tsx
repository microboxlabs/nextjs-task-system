...
export default function ListViewTasks({ tasks,setShowToast,setTasksData }: Props) {
    const { setUpdateModal, setShowModal,setIdForDelete, idForDelete,setShowDeleteModal } = useTaskContext();
                ...
                <Table className=" min-w-full">
                    ...
                    <Table.Body className="divide-y">
                        {tasks
                            ...
                            .map((task, index) => (
                                <Table.Row key={`person-${index}`} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                    ...
                                    <Table.Cell>
                                        <button onClick={()=> handleEditClick(task)} className="font-medium text-cyan-600 hover:underline dark:text-cyan-500">
                                            Edit
                                        </button>
                                    </Table.Cell>
                                    <Table.Cell>
                                        <button onClick={()=> handleDeleteClick(task.id)} className="font-medium text-red-600 hover:underline dark:text-red-500">
                                            Delete
                                        </button>
                                    </Table.Cell>
                                </Table.Row>
                ...
                <Table className="min-w-full">
                    ...
                    <Table.Body className="divide-y">
                        {tasks
                            .filter((task) => task.group != null)
                            .map((task, index) => (
                                <Table.Row key={`group-${index}`} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                    <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">
                                        {task.title}
                                    </Table.Cell>
