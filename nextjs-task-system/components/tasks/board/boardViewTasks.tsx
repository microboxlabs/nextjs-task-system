    ...
    <div className="overflow-x-auto p-4 w-full">
      ...
      <div className="flex gap-4 w-max">
        {Object.entries(columns).map(([status, columnTasks]) => (
          <div className="w-[300px]" key={status}> {/* Ancho fijo para cada columna */}
            <Column title={status} tasks={columnTasks} setTasksData={setTasksData} setShowToast={setShowToast} />
          </div>
        ))}
      </div>
    </div>
