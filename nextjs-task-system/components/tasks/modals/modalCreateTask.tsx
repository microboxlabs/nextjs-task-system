    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (cacheRef.current[typeOfAssigned]) {
                ...
                try {
                    const url = process.env.NEXT_PUBLIC_URL_PAGE + "/api/tasks/getselects" + (typeOfAssigned ? `?${typeOfAssigned}` : "");
                    const response = await fetch(url, {
                        method: "GET",
                        credentials: "include",
                    });
                    const data: ResponseSelectAssigned = await response.json();
                    cacheRef.current[typeOfAssigned] = data;
                        ...
                        <form action={async (formData) => {
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <Select onChange={value => setTypeOfAssigned(value.target.value)} name="TypeOfAssigned" id="typeOfAssigned" required>
                                        <option value="">Select a value</option>
                                ...
                                {typeOfAssigned != '' && (
                                    <div>
                                        <div className="mb-2 ">
                                            <Label htmlFor={typeOfAssigned == "person" ? `perPerson` : `perGroup`} value={typeOfAssigned == "person" ? `Person assigned` : `Group assigned`} />
                                        </div>
                                        <Select
                                ...
                                <div className="col-span-2 mb-2">
                                    ...
                                    <Textarea id="description" name="Description" placeholder="Describe your task" required />
                                </div>
                            </div>
